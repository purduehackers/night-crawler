...
function createUser(discordUid, discordUsername, avatarURL) {
  return new Promise((resolve, reject) => {
    const tempUsername = discordUsername
      .replace(/[^a-zA-Z0-9]/g, '')
      .concat('-', uid()) // temp username before user sets their own
    // const tempUsername = discordTag.replace(/[^a-zA-Z0-9]/g, '');
    if (!avatarURL)
      avatarURL = `https://avatars.dicebear.com/api/jdenticon/${tempUsername}.svg` // https://avatars.dicebear.com/docs/http-api
